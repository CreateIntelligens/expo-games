...
import GestureSessionService from './gesture-session-service.js';
import GesturePresenter from './gesture-presenter.js';
...
export class GestureController {
    ...
    async initialize() {
        if (this.isInitialized) {
            console.log('🎨 GestureController 已初始化');
            return;
        }
        ...
        try {
            ...
            console.error('❌ GestureController 初始化失敗:', error);
            this.statusManager.update(`初始化失敗: ${error.message}`, STATUS_TYPES.ERROR);
            throw error;
        }
    }
    ...
    async initializePreview() {
        try {
            ...
            this.statusManager.update('攝影機預覽已就緒，請點擊「開始手勢繪畫」', STATUS_TYPES.SUCCESS);
    ...
    async startDrawing() {
        try {
            ...
            this.statusManager.update('啟動手勢繪畫...', STATUS_TYPES.INFO);
    ...
    async stopDrawing(closeCamera = false) {
        try {
            ...
            if (closeCamera) {
                ...
                this.statusManager.update('手勢繪畫已完全停止', STATUS_TYPES.INFO);
            } else {
                // 顯示最終結果
                if (finalImage) {
                    ...
                    this.statusManager.update('繪畫已完成，顯示最終作品', STATUS_TYPES.SUCCESS);
                } else {
                    this.statusManager.update('繪畫已停止', STATUS_TYPES.INFO);
                }
            }
    ...
    async clearCanvas() {
        try {
            ...
            if (!this.isActive) {
                this.statusManager.update('請先開始繪畫會話', STATUS_TYPES.WARNING);
                return;
            }
            ...
            this.statusManager.update('畫布已清空', STATUS_TYPES.SUCCESS);
    ...
    async changeColor(colorName) {
        try {
            ...
            this.statusManager.update(`顏色已變更為 ${colorName}`, STATUS_TYPES.SUCCESS);
    ...
    async changeBrushSize(size) {
        try {
            ...
            this.statusManager.update(`筆刷大小已變更為 ${size}`, STATUS_TYPES.SUCCESS);
    ...
    async startNewDrawing() {
        try {
            ...
            this.statusManager.update(`開始新繪畫失敗: ${error.message}`, STATUS_TYPES.ERROR);
        }
    }
    ...
    async handleCameraReady({ stream, videoSize } = {}) {
        ...
        if (videoElement) {
            try {
                ...
                this.statusManager.update(`攝影機綁定失敗: ${error.message}`, STATUS_TYPES.ERROR);
            }
        }
    }
    ...
    handleSessionStarted({ sessionId }) {
        ...
        this.statusManager.update('手勢繪畫已啟動，開始偵測手勢', STATUS_TYPES.SUCCESS);
    }
    ...
    handleRecognitionResult({ shape, confidence, message }) {
        ...
        if (message) {
            this.statusManager.update(message, STATUS_TYPES.SUCCESS);
        }
    }
    ...
    handleColorChanged(data) {
        ...
        this.statusManager.update(`顏色已切換為 ${color}`, STATUS_TYPES.SUCCESS);
    }
    ...
    handleCanvasCleared() {
        ...
        this.statusManager.update('畫布已清空', STATUS_TYPES.INFO);
    }
    ...
    handleSessionError(errorDetail) {
        ...
        this.statusManager.update(`會話錯誤: ${message}`, STATUS_TYPES.ERROR);
    }
    ...
    destroy() {
        ...
        // 停止所有活動
        this.stopFrameProcessing();
        this.stopDrawing(true).catch(console.error);
