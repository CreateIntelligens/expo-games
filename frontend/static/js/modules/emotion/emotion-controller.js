...
import { EmotionPresenter } from './emotion-presenter.js';
...
export class EmotionController {
    ...
    setupEventListeners() {
        ...
        this.transportSubscriptions.push(this.transport.on('close', () => {
            if (this.isDetecting) {
                this.statusManager.update('WebSocket連線已關閉', STATUS_TYPES.WARNING);
            }
        }));
    }
    ...
    async initializeCameraPreview() {
        if (this.cameraService.isActive()) {
            this.statusManager.update('攝影機預覽已啟動', STATUS_TYPES.INFO);
            return;
        }
        ...
        try {
            ...
            this.statusManager.update('攝影機預覽已就緒，請點擊「開始情緒檢測」開始分析', STATUS_TYPES.SUCCESS);
            console.log('✅ 攝影機預覽初始化完成');

        } catch (error) {
            this.statusManager.update(`攝影機預覽初始化錯誤：${error.message}`, STATUS_TYPES.ERROR);
        }
    }
    ...
    handleWebSocketResult(data) {
        ...
        switch (data.type) {
            ...
            case 'error':
                ...
                this.statusManager.update(`分析錯誤: ${errorMsg}`, STATUS_TYPES.ERROR);
                break;
